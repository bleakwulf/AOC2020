let inputDemo = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
let inputDemo02 = `shiny gold bags contain 2 dark red bags.
...
let input007 = `muted lime bags contain 1 wavy lime bag, 1 vibrant green bag, 3 light yellow bags.
...
const solutionD07P01 = rawInput => {
  let input = rawInput.replace(/(?:bags\.|bag\.|bags, |bag, |bags |bag )/g, '')
  let allOuterBags = []
  let isFindMoreOuterBags = true
  let lastOuterBagsFound
  
  do {
    let childBags = !allOuterBags.length ? targetBag : lastOuterBagsFound.join(`|`)
    let pattern = new RegExp(`^([\\da-z ,]+)(?:${childBags})([\\da-z ,\\.]+)$`, `gm`)
    let matches = input.match(pattern)

    if (!matches) isFindMoreOuterBags = false
    else {
      let parentBags = lastOuterBagsFound = matches.map( rule => {
        let [parentBag] = rule.split(' contain')
        return parentBag
      })
      isFindMoreOuterBags = !parentBags.every(parentBag => allOuterBags.includes(parentBag))
      if (isFindMoreOuterBags) allOuterBags = [...new Set( allOuterBags.concat(parentBags))]
    }
  } while( isFindMoreOuterBags )

  console.log(`\nDay 07 Part 01: ${allOuterBags.length}`)
}

const solutionD07P02 = rawInput => {
  
  let input = rawInput.replace(/(?: bags\.| bags| bag| bag\.)/g, '')
  input = input.replace(/(?:\scontain\s|\scontains\s)/g, `|`)
  input = input.replace(/no other/g, `0`)
  
  let allInnerBags = [ [ targetBag, 1 ] ]
  let isFindMoreInnerBags = true
  let totalInnerBags = 0

  do {
    let [ parentBag, qty ] = allInnerBags[0]
    let pattern = new RegExp(`^${parentBag}([\|\\da-z ,\\.]+)$`, `gm`)
    let matches = input.match(pattern)
    
    totalInnerBags += parentBag !== targetBag ? +qty : 0
    
    if (matches) {
      let childBags = matches.map( rule => {
        let [, innerBagsQty] = rule.split(`|`)
        return innerBagsQty
          .split(`, `)
          .map( innerBagQty => {
            let [ qty2, trait, color ] = innerBagQty.split(' ')
            return +qty2 ? [ `${trait} ${color}`, +qty2 * +qty ] : []
          })
        .filter( innerBagQty => innerBagQty.length )
      })
      allInnerBags = allInnerBags.concat(...childBags)
    }
    
    allInnerBags.shift()
    
    isFindMoreInnerBags = allInnerBags.length
    
  } while( isFindMoreInnerBags )
  
  console.log(`\nDay 07 Part 02: ${totalInnerBags}`)
}

solutionD07P01(inputDemo)
solutionD07P02(inputDemo)
solutionD07P02(inputDemo02)

solutionD07P01(input007)
solutionD07P02(input007)