let inputDemo01 = `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
let inputDemo02Pass = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
let inputDemo02Fail = `eyr:1972 cid:100
...
const toObjectArray = rawInput => {
  return rawInput
    ...
    .map(propsArray => {
      return propsArray.reduce((passport, props) => {
        let [key, value] = props
        passport[key] ??= value
...
const validate = (field, value) => {
  ...
  switch (field) {
    case `byr`:
      ...
      result = +value >= 1920 && +value <= 2002
      break
    case `iyr`:
      //  Issue Year - four digits; at least 2010 and at most 2020
      result = +value >= 2010 && +value <= 2020
      break
    case `eyr`:
      //  Expiration Year - four digits; at least 2020 and at most 2030
      result = +value >= 2020 && +value <= 2030
      break
    case `hgt`:
    ...
    case `hcl`:
      ...
      result = !!value.match(/^#[0-9a-f]{6}/)?.length
      break
    case `ecl`:
      //  Eye Color - exactly one of: amb blu brn gry grn hzl oth.
      result = [`amb`, `blu`, `brn`, `gry`, `grn`, `hzl`, `oth`].includes(value)
      break
    case `pid`:
...
const solutionD04P01 = rawInput => {
  let input = toObjectArray(rawInput)
  let validPassports = input.reduce((validPassports, currentPassport) => {
    let missingFields = propsList.reduce((missingProps, currentProp) => {
      if (!currentPassport[currentProp]) missingProps.push(currentProp)
      return missingProps
    }, [])

    if (!missingFields.length) validPassports++
    if (missingFields.length === 1 && missingFields[0] === `cid`) validPassports++

    return validPassports
  }, 0)
  console.log(`\nDay 04 Part 01: ${validPassports}`)
}

const solutionD04P02 = rawInput => {
  let input = toObjectArray(rawInput)
  let validPassports = input.reduce((validPassports, currentPassport) => {
    let missingFields = propsList.reduce((missingProps, currentProp) => {
      if (!currentPassport[currentProp]) missingProps.push(currentProp)
    ...
    let doValidation = (!missingFields.length) || (missingFields.length === 1 && missingFields[0] === `cid`)

    if (doValidation) {
      let isPassed = true

      for (i = 0; i < propsList.length; i++) {
        isPassed = validate(propsList[i], currentPassport[propsList[i]])
        // console.log(`${propsList[i]}: ${currentPassport[propsList[i]]} => ${isPassed}`)
        if (!isPassed) break
      }

      if (isPassed) validPassports++
    }

    return validPassports
  }, 0)
  console.log(`\nDay 04 Part 02: ${validPassports}`)
}

solutionD04P01(inputDemo01)
solutionD04P02(inputDemo02Pass)
solutionD04P02(inputDemo02Fail)

solutionD04P01(input004)
solutionD04P02(input004)