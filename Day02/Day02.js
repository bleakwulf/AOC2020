let inputDemo = `1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc`

let input002 = `2-7 p: pbhhzpmppb
3-6 h: jkhnhwhx
5-6 x: xxxxxmxf
...
const solutionD02P01 = rawInput => {
  let input = rawInput
    ?.trim()
    .split(`\n`)
  
  let validPasswords = input.reduce(
    ( previous, current ) => {
      if (current) {
        let [ policy, password ] = current
          .trim()
          .split(`: `)
  
        let [ min, max, pattern ] = policy
          .replace(/-/g, ' ')
          .split(' ')

        let patternMatcher = new RegExp(pattern, "g")
        let policyMatch = password.match(patternMatcher)
        if ( policyMatch?.length >= min && policyMatch?.length <= max ) {
          /* console.log(`min: ${min}`)
          console.log(`max: ${max}`)
          console.log(`pattern: ${pattern}`)
          console.log(`password: ${password}`) */
          previous.push( [ min, max, pattern, password ] )
        }
      }
  ...
  let validPasswords = input.reduce(
    ( previous, current ) => {
      if (current) {
        ...
        let check1 = password.at(min - 1) === pattern
        let check2 = password.at(max - 1) === pattern
...
solutionD02P01(inputDemo)
solutionD02P02(inputDemo)

solutionD02P01(input002)
solutionD02P02(input002)