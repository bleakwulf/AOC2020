...
const solutionD06P01 = rawInput => {
  let totalYesQuestions = rawInput
    ?.trim()
    .replace(/\s\s/g, `|`)
    .replace(/\s/g, ``)
    .split(`|`)
    .map( yesQuestions => [...new Set(yesQuestions)].length)
    .reduce( (totalYesQuestions, currentQ) => totalYesQuestions + currentQ, 0)
  
  console.log(`\nDay 06 Part 01: ${totalYesQuestions}`)
}

const solutionD06P02 = rawInput => {
  let allYesPerGroup = rawInput
    ?.trim()
    .split(`\n\n`)
    .map( answersData => answersData.split(`\n`))

  let allUniqueYesPerGroup = allYesPerGroup.map( yesQuestions => [...new Set(yesQuestions.join(''))].sort())

  let yesCount = allUniqueYesPerGroup.reduce( 
    ( allYesFromGroups, currentGroup, groupIndex ) => {
      let allYesFromUsers = currentGroup.reduce( 
        ( allAnsweredYes, currentQ, qIndex ) => {
          let currentGroupAnswers = allYesPerGroup[groupIndex].join('')
          let groupUserCount = allYesPerGroup[groupIndex].length
           
          let qPattern = new RegExp(currentQ, "g")
          let answerMatch = currentGroupAnswers.match(qPattern)
  
          if (answerMatch?.length === groupUserCount)
            allAnsweredYes.push(currentQ)
  
          return allAnsweredYes
        }, [])

      return allYesFromGroups += allYesFromUsers.length
    }, 0)
  
  console.log(`\nDay 06 Part 02: ${yesCount}`)
}

solutionD06P01(inputDemo)
solutionD06P02(inputDemo)

solutionD06P01(input006)
solutionD06P02(input006)